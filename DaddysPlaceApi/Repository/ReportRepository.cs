...
namespace DaddysPlaceApi.Repository
{
    public class ReportRepository : IReportRepository
    {
        ...
        {
            string sqlString = "SELECT  Bill.Id, Bill.CreateOn, Bill.Frn_UserId,  Bill.TotalAmount As BillTotal,[User].email, PaymentType.PaymentType, (select sum(Bill.TotalAmount) from Bill ) As TotalAmount FROM Bill INNER JOIN PaymentType ON Bill.Frn_PaymentId = PaymentType.Id INNER JOIN [User] ON Bill.Frn_UserId = [User].Id where Bill.CreateOn between @StartDate and @EndDate ";
